...
export default function SelectBase(props) {
  ...
  return (
    <div className="relative h-12 w-full">
      <select
        className={clsx(
          "peer h-full w-full rounded-md border bg-transparent px-3 py-3 text-base font-normal outline outline-0 transition-all placeholder-shown:border focus:border-2 focus:outline-0  disabled:bg-blue-gray-50 placeholder-shown:placeholder:text-transparent placeholder-shown:focus:placeholder:text-blue-gray-100 text-blue-gray-800",
          error
            ? "border-red-200 focus:border-red-500 placeholder-shown:border-red-200 placeholder-shown:border-t-red-200"
            : "border-blue-gray-200 focus:border-blue-500 placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200",
      ...
      >
        {options.map((item, i) => (
          <option key={i} value={mapValue(item)} {...optionProps}>
            {mapLabel(item)}
          </option>
        ))}
      </select>
